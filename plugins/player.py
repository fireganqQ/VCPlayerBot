...
async def add_to_playlist(_, message: Message):
    with suppress(MessageIdInvalid, MessageNotModified):
        ...
        if message.reply_to_message and message.reply_to_message.video:
            await msg.edit("⚡️ **Telegram Media kontrol ediliyor...**")
            type='video'
        ...
        if " " in m.text:
            ...
            if me.startswith("-100"):
                ...
                try:
                    ...
                    await k.edit(f"Verilen kanal özel ve @{Config.BOT_USERNAME} orada bir yönetici değil. Kanal gizli değilse lütfen kanalın kullanıcı adını belirtin.")
                    await delete_messages([m, k])
                    return
                except UserNotParticipant:
                    LOGGER.error("Verilen kanal özeldir ve KULLANICI hesabı kanala üye değildir.Verilen kanal özeldir ve KULLANICI hesabı kanala üye değildir.")
                    await k.edit("Verilen kanal özeldir ve KULLANICI hesabı kanala üye değildir.")
                    await delete_messages([m, k])
                    return
                except Exception as e:
                    LOGGER.error(f"Kanalla ilgili veriler alınırken hatalar oluştu - {e}", exc_info=True)
                    await k.edit(f"Bir şeyler ters gitti- {e}")
                    await delete_messages([m, k])
                    return
                ...
                if st == False:
                    ...
                    await k.edit(f"{msg} dosyaları oynatma listesine başarıyla eklendi.")
            elif me.startswith("@"):
                me = me.replace("@", "")
                try:
                ...
                if st == False:
                    ...
                    await k.edit(f"{chat.title} dosyasından oynatma listesine {msg} dosyası başarıyla eklendi")
                    await delete_messages([m, k])
            else:
                await k.edit("Verilen kanal geçersiz. Özel kanallar için -100 ile başlamalı ve genel kanallar için @\nÖrnekler - `/cplay @VCPlayerFiles veya /cplay -100125369865 ile başlamalıdır\n\nÖzel kanal için hem bot hem de KULLANICI hesabı kanalın üyesi olmalıdır.")
                await delete_messages([m, k])
        else:
            await k.edit("Bana kanal vermedin. Dosyaları oynatmam gereken bir kanal kimliği veya kullanıcı adı verin. \nÖzel kanallar için -100 ile, genel kanallar için @ ile başlamalıdırÖrnekler - `/cplay @VCPlayerFiles veya /cplay -100125369865\n\nÖzel kanal için hem bot hem de KULLANICI hesabı kanalın üyesi olmalıdır.")
            await delete_messages([m, k])
...
async def stream(client, m: Message):
    with suppress(MessageIdInvalid, MessageNotModified):
        ...
        if match:
            ...
            if not stream_link:
                k = await msg.edit("Bu geçersiz bir bağlantı.")
                await delete_messages([m, k])
                return
        else:
            stream_link=link
        ...
        Config.msg['player']=await msg.edit(f"[Akış]({stream_link}) Başladı. ㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤ", disable_web_page_preview=True, reply_markup=await get_buttons())
        await delete_messages([m])
...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAADBQADEgQAAtMJyFVJOe6-VqYVzAI", caption="Yetkili değilsin", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⚡️Join Here', url='https://t.me/subin_works')]]))
    await delete_messages([m, k])
