...
async def schedule_vc(bot, message):
    with suppress(MessageIdInvalid, MessageNotModified):
        ...
        if message.chat.type!='private' and message.from_user is None:
            await msg.edit(
                text="Anonim bir yönetici olduğunuz için buradan program yapamazsınız. Özel sohbet yoluyla planlamak için planla düğmesini tıklayın.",
...
async def list_schedule(bot, message):
    ...
    if not Config.SCHEDULE_LIST:
        await k.edit("Oynamak için programlanmış bir şey yok.")
...
async def delete_sch(bot, message):
    with suppress(MessageIdInvalid, MessageNotModified):
        m = await message.reply("Planlanmış akışı bulma..")
        if " " in message.text:
        ...
        await sync_to_db()
        await m.edit(f"{data['1']} planlanmış listeden başarıyla silindi.")
        await delete_messages([message, m])
...
async def delete_all_sch(bot, message):
    ...
    reply_markup = InlineKeyboardMarkup(buttons)
    await message.reply("Planlanan tüm akışları iptal etmek istiyor musunuz?", reply_markup=reply_markup)
    await delete_messages([message])
